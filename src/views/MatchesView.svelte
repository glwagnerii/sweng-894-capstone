...
<div id="view-matches" class="p-4">
  ...
  {:else if $mealsQuery?.error}
    ...
    {:else if $mealsQuery?.data?.meals?.length}
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 justify-items-center" data-testid="recipe-view">
        {#each $mealsQuery.data.meals as recipe (recipe.idMeal)}
          <button
            type="button"
            class="flex bg-base-200 items-center rounded-xl p-3 shadow-md w-full max-w-[334px] text-left hover:bg-base-300 transition"
